...
import 'package:web3plugin_setheum/pages/loan/loanInfoPanel.dart';
...
class _LoanAdjustPageState extends State<LoanAdjustPage> {
    ...
    final bool res = await showCupertinoDialog(
        ...
        builder: (_) {
          return CupertinoAlertDialog(
            content: Text(dic[
                'loan.warn${widget.plugin.basic.name == plugin_name_setheum ? '.KSM' : ''}']),
            actions: <Widget>[
  ...
  Widget build(BuildContext context) {
    ...
    BigInt balanceStableCoin = Fmt.balanceInt(
        widget.plugin.store.assets.tokenBalanceMap[karura_stable_coin].amount);
    ...
    final debitsView = Fmt.priceCeilBigInt(loan.debits, stableCoinDecimals);
